...
type 'entry choice = { entry_pair: 'entry option * 'entry option; winner: 'entry option }

type ichoice = { ientry_pair: int option * int option; winner: int option }

type round =  ichoice array
type 'entry tourney = { rounds: round array; entries: 'entry list }
type 'entry round_in_progress = 'entry choice list
...
let to_string tourney pfunc =
  List.fold_left (fun str entry -> str ^ (pfunc entry) ^ "\n")
	"" tourney.entries 
...
let print_by_entry tourney entry_to_string container =
  ...
  let do_entry_choices i choices =
	let entry_str =
 	  (match List.hd choices with { entry_pair = (Some a, _) } ->
		entry_to_string a | _ -> failwith "bug") in
	...
	let do_choice i choice =
	  let () = match choice with
		| { entry_pair = Some a, Some b; winner = Some c } ->
		  ...
		  Jsutil.addTd row (entry_to_string b) None;
...
let print_by_round tourney entry_to_string outer_container =
  ...
  let iter i (undecided, decided) =
	...
	let riter i choice = 
	  ...
	  | { entry_pair = Some a, Some b; winner = Some c } ->
		...
		Jsutil.addTd row (entry_to_string c) None;
		Jsutil.addTd row "defeated" (Some "tourney-won");
		Jsutil.addTd row (entry_to_string loser) None;
	  | { entry_pair = Some a, Some b; winner = None } ->
		Jsutil.addTd row (entry_to_string a) None;
...
let show tourney entry_to_string =
  ...
  add inner;
  print_by_round tourney entry_to_string inner;
  ...
  let clicks positive negative print =
	...
	Lwt.return (print tourney entry_to_string inner))
  in
